      ...
      <table class="table table-sm align-middle w-100" id="tabela">
        ...
        <?php } else { ?>
          ...
          <?php foreach ($rows as $r) {
            ...
            $nome     = esc($r['nome'] ?? '');
            $email    = esc($r['email'] ?? '');
            $telefone = esc($r['telefone'] ?? '');
            $foto     = fotoUrl($r['foto'] ?? 'sem_foto.webp');

            $ativoRaw = (string)($r['ativo'] ?? 'Sim');
            $ativoEhNao = (strtolower($ativoRaw) === 'não' || strtolower($ativoRaw) === 'nao');

            $badgeAtivo = $ativoEhNao
          ...
          ?>
            <tr>
              ...
              <td><?= $email !== '' ? $email : '<span class="text-muted">—</span>' ?></td>
              <td><?= $telefone !== '' ? $telefone : '<span class="text-muted">—</span>' ?></td>
              <td><?= $badgeAtivo ?></td>
