<?php
/**
 * scripts/resetar_senha_cliente.php
 * Redefinição de senha para CLIENTE
 */
...
if ($token === '' || strlen($token) > 200) {
    ...
    // Busca tokens válidos de CLIENTE
    $stmt = $pdo->prepare("
        SELECT id, cliente_id, token_hash, expira_em, usado_em
    ...
    if (!$tokenRow) {
        ...
        $stmtC = $pdo->prepare("
            ...
            LIMIT 1
        ");
        $stmtC->execute([':id' => (int)$tokenRow['cliente_id']]);
        $cliente = $stmtC->fetch(PDO::FETCH_ASSOC);

        if (!$cliente) {
            $erro = 'Cliente não encontrado.';
        } elseif (strtoupper(trim((string)$cliente['ativo'])) !== 'SIM') {
            $erro = 'Conta inativa. Contate o suporte.';
        } else {
...
if ($mostrarFormulario && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (strlen($senha) < 6 || strlen($senha) > 256) {
        ...
        try {
            ...
            // Invalida outros tokens do cliente
            $pdo->prepare("
                UPDATE recuperacao_senha
                SET usado_em = NOW()
                WHERE cliente_id = :cid
                  AND usado_em IS NULL
            ")->execute([':cid' => (int)$cliente['id']]);

            $pdo->commit();

            $sucesso = 'Senha redefinida com sucesso! Você já pode acessar sua conta.';
            $mostrarFormulario = false;
        ...
        <div class="card-body p-4">
          ...
          <?php if ($sucesso): ?>
            ...
            <div class="text-center">
              <a href="../acesso.php" class="btn btn-primary mt-2">Ir para o login</a>
            </div>
          <?php endif; ?>

          <?php if ($mostrarFormulario): ?>
            <p class="text-muted mb-3">
              Olá <strong><?= htmlspecialchars($cliente['nome'] ?: 'cliente') ?></strong>, escolha sua nova senha.
            </p>
