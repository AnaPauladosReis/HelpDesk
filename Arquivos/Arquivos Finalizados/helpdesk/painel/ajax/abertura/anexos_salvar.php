...
try {
  ...
  // 🔒 Permissão (ajuste se quiser: criar/editar)
  if (!podeFazer('editar') && !podeFazer('criar')) {
    out(false, 'Sem permissão para anexar arquivos.');
  }
  ...
  $cliente_id = isset($ch['cliente_id']) ? (int)$ch['cliente_id'] : 0;
...
$st = $pdo->prepare("
  ...
  (chamado_id, usuario_id, cliente_id, arquivo, nome, criado_em)
  VALUES 
  (:chamado_id, :usuario_id, :cliente_id, :arquivo, :nome, NOW())
");
...
$st->execute([
  ...
  // log
  registrarLog($pdo, 'inserir', 'chamados_anexos', $anexoId, "Anexo enviado no chamado {$protocolo}");

  out(true, 'Arquivo anexado com sucesso!', [
    'id' => $anexoId,
    'arquivo' => $arquivoFinal,
    'nome' => $nome
  ]);
