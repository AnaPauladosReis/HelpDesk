...
try {
    ...
    // confirma existência
    $stmt = $pdo->prepare("SELECT id FROM {$tabela} WHERE id = :id LIMIT 1");
    $stmt->execute([':id' => $id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$row) {
        echo json_encode(['ok' => false, 'msg' => 'Cargo não encontrado.'], JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    echo json_encode(['ok' => true, 'msg' => 'Cargo excluído com sucesso!'], JSON_UNESCAPED_UNICODE);
