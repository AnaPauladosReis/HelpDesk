...
<script>
  const URL_BASE = <?= json_encode($url_sistema) ?>;
</script>
...
(function(){
  ...
  form.addEventListener('submit', async function(e){
    ...
    try{
      ...
      const respCli = await fetch(URL_BASE + "ajax/cliente_resolver.php", {
        method:"POST", body: fdCli, cache:"no-store"
      });     
      ...
      const resp = await fetch(URL_BASE + "painel/ajax/abertura/salvar.php", {
  method: "POST",
  body: fd,
  cache: "no-store"
});
  ...
  async function buscarClientePorDoc(docSomenteNumeros){
    const resp = await fetch(`${URL_BASE}ajax/cliente_buscar.php?cpf_cnpj=${encodeURIComponent(docSomenteNumeros)}`, {
  cache: "no-store"
});
...
(function(){
  ...
  function mascaraTelefone(input){
    let v = soNumeros(input.value).slice(0, 11); // no máx 11 dígitos (DDD + 9)
    if (!v) { input.value = ""; return; }

    // se tiver DDI 55 digitado, remove (opcional)
    if (v.length >= 12 && v.startsWith("55")) v = v.slice(2);

    const ddd = v.slice(0, 2);
    const resto = v.slice(2);

    // (99) 99999-9999 (11 dígitos) ou (99) 9999-9999 (10 dígitos)
    if (resto.length >= 9) {
      input.value = `(${ddd}) ${resto.slice(0,5)}-${resto.slice(5,9)}`;
    } else if (resto.length >= 5) {
      input.value = `(${ddd}) ${resto.slice(0,4)}-${resto.slice(4,8)}`;
  ...
  if (telInp){
    telInp.addEventListener("input", ()=> mascaraTelefone(telInp));
    telInp.addEventListener("blur",  ()=> mascaraTelefone(telInp));
    // opcional: ao colar, já formata
    telInp.addEventListener("paste", ()=> setTimeout(()=> mascaraTelefone(telInp), 0));
  }
