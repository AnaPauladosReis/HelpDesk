...
function fotoUrl($tipo, $fotoArquivo){
  global $url_sistema; // usa a variável já montada no conexao

  $baseUrl = rtrim($url_sistema, '/') . '/uploads/';
  ...
  if (!$fsBase) {
    return ($tipo === 'usuario') ? $fallbackUsuario : $fallbackCliente;
  }
...
<div class="chat-wrap" data-chamado-id="<?= (int)$chamado_id ?>">
  ...
  ?>
    ...
    <?php } else { ?>
      ...
      <div class="chat-row <?= $lado === 'right' ? 'is-other' : 'is-me' ?>">
